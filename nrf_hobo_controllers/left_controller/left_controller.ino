...
void setup() {
  ...
  printf_begin();
  while (!Serial) {}
  const char* device_name = "nrf24_vr_left_controller";  // used only for debugging
  Serial.println(device_name);
  Serial.println("debug setup complete");
  
  //radio
  radio.begin();
  radio.openWritingPipe(left_controller_talking_pipe);
  radio.openReadingPipe(1,left_controller_listening_pipe);
